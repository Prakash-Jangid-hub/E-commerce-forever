...
``
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
        ...
        <div className='flex gap-2'>
          {images.map((img, index) => (
            <label key={index} htmlFor={`image${index}`} className='cursor-pointer'>
              ...
              <input accept='image/*' onChange={(e) => handleImageChange(e, index)} type="file" id={`image${index}`} hidden />
            </label>
          ))}
      ...
      <button type='submit' className='w-28 cursor-pointer py-3 mt-4 bg-black text-sm text-white'>ADD</button>
    </form>
  )
}
